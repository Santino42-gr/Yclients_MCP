    ...
    async handleBookAppointment(args) {
        try {
            ...
            if (request.master_name) {
                ...
                const availableStaff = allStaff.filter(staff => !staff.services || staff.services.includes(service.id));
    ...
    async handleFindAvailableTime(args) {
        try {
            ...
            if (request.master_name) {
                ...
                targetStaff = allStaff.filter(staff => !staff.services || staff.services.includes(service.id));
            }
            ...
            for (const staff of targetStaff) {
                for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
                    ...
                    try {
                        const schedule = await this.apiClient.getAvailableTime(staff.id, service.id, dateStr);
